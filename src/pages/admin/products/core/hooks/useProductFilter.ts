...
import { saveProductFilter, getProductFilters, deleteProductFilter, updateProductFilter } from '../_request'
...
// Save a new filter
export const useSaveProductFilter = () => {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (filter: SavedFilter) => saveProductFilter(filter),
    onSuccess: (response) => {
...
// Delete a filter
export const useDeleteProductFilter = () => {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: (id: string) => deleteProductFilter(id),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: filterKeys.lists() })
